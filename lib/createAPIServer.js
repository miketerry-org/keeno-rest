...
const { Schema, types } = require("keeno-schema");

// Destructure required validators
const { Boolean, integer, string } = types;

// Default logger fallback (native console)
const createDefaultLogger = () => console;

function createAPIServer(options = {}) {
  // Validate environment configuration
  const { object: config, errors } = new Schema({
  ...
  const bodyLimit = config.body_limit || "10kb";
  ...
  // Logger setup
  const logger =
    typeof options.createLogger === "function"
      ? options.createLogger(config)
      : createDefaultLogger();
